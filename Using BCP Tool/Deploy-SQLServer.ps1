...
try
{
    ...
    if(($Config.SubscriptionName -ne $null) -and ($Config.ResourceGroup -ne $null))
    {
        ...
        foreach($item in $Config.DBCSVs)
        {
            ...
            $DBName = $Config.DataBaseSchema+"."+$item.split("\")[-1].replace(".csv","")
            BCP $DBName IN $item -S "$($Config.DataBaseServer).database.windows.net" -d $Config.DataBaseName  -U $Sqlcred.UserName -P $Sqlcred.GetNetworkCredential().Password -c -t"," -m 100000
        }
